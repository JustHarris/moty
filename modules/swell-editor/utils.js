...
const settingPaths = {
  bodyFont: 'typography.base',
  bodyMediumFont: 'typography.medium',
  bodySemiboldFont: 'typography.semibold',
  bodyBoldFont: 'typography.bold',
...
export function getGoogleFontConfig(settings) {
  ...
  const fonts = [
    get(normalizedSettings, settingPaths.bodyFont, {}),
    get(normalizedSettings, settingPaths.bodyMediumFont, {}),
    get(normalizedSettings, settingPaths.bodySemiboldFont, {}),
    get(normalizedSettings, settingPaths.bodyBoldFont, {})
  ]
...
function getCssVariables(settings) {
  ...
  const isFontSetting = varName =>
    [
      toVarName(settingPaths.bodyFont),
      toVarName(settingPaths.bodyMediumFont),
      toVarName(settingPaths.bodySemiboldFont),
      toVarName(settingPaths.bodyBoldFont)
    ].includes(varName)
  ...
  return getVariableGroups().reduce((variables, groupName) => {
    ...
    for (const [key, value] of Object.entries(properties)) {
      ...
      if (isRatioSetting(varName)) {
        ...
        const ratio = (parseFloat(value) || 1.25) - 1
        const steps = range(-6, 24) // Generate a reasonable range for the scale
        steps.map(step => {
          // const typeSizeValue = round(ratio ** step * (base / 16), 3) + 'rem'
          const typeSizeValue = 1 + round(ratio * step * (base / 16), 3) + 'rem'
...
function flattenGroup(groupValue, groupName) {
  const isFontSetting = name =>
    [
      settingPaths.bodyFont,
      settingPaths.bodyMediumFont,
      settingPaths.bodySemiboldFont,
      settingPaths.bodyBoldFont
