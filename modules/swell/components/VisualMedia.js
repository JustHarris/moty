...
export default {
  ...
  render(h, context) {
    ...
    const imgClass = isBackground
      ? 'absolute top-0 left-0 w-full h-full ' + imgExtraClass
      : 'absolute inset-0 w-full h-full ' + imgExtraClass

    if (!onlyGetImg) {
      return (
        <div class={wrapperClass} style={isBackground ? null : `padding-bottom: ${ratioPadding}`}>
          <img {...{ attrs: image }} style={styleInlineImg} class={imgClass} />
        </div>
      )
    } else {
      return <img {...{ attrs: image }} style={styleInlineImg} class={imgExtraClass} />
    }
  }
}
